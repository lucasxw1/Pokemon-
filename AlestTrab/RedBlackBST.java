...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
    ...
    */
    private Node addAux(Node h, Key key, Value val) { 
        ...
        if      (cmp < 0) h.left  = addAux(h.left,  key, val); // Caso a key seja maior que o nodo h passado, chama o metodo para o lado esquerdo da arvore
        else if (cmp > 0) h.right = addAux(h.right, key, val); // Caso a key seja menor que o nodo h passado, chama o metodo para o lado direito da arvore;
        else              h.val   = val; // Caso a key ja exista na arvore, substitui o valor dela;


        //Checa as propriedades da arvore e faz as rotacoes necessarias pra balancear ela;
